...
#define STOP_TIMER(X,CHAR_DEST) do{tmpClock[X]=(double)(clock()-c_starts[X])/((double)CLOCKS_PER_SEC);sprintf(CHAR_DEST,"%f",tmpClock[X]);}while(0)
...
std::vector<std::string> transTypeStrings = {
	"NONE",
	"ICP",
	"NDT",
	"ICP_NL_NORM",
...
std::vector<std::string> transDescStrings = {
...
int getTranstype(const std::string & transStr)
{
	int transType = 0;
	for(auto & type:transTypeStrings )
	{
		if (transStr==type) break;
		transType++;
	}
	return transType;
}
...
{
	for (int i=0;i<transTypeStrings.size();i++)
	{
...
int main (int argc, char ** argv)
{
	...
	bool hasGps=false;
	bool isGpsTransType=false; //whether or not transType is GPS enchanced
	...
	if (find_switch (argc, argv, "-t") )
	{
		...
		//check if transStr is Gps Type
		std::string transStr_lower;
		std::transform(transStr.begin(), transStr.end(), transStr_lower.begin(), ::tolower);
		isGpsTransType = (transStr_lower.find("gps")!=std::string::npos); //found gps in transStr

		if(transType>=transTypeStrings.size())
		{
			std::cerr<<"! Error: Unknown transform type: "<<transStr<<std::endl;
			usage (argv);
			return 1;
		}
	}
	...
	else if (isGpsTransType)
	{
		std::cerr << "! Error: Cannot run type GPS without kml gps input file <-gps [path-to-kml]>"<< endl;
		usage(argv);
		return(0);
	}
