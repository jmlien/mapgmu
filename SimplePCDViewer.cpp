...
public:
  ...
  SimplePCDViewer (std::string file,
    ...
    bool isDir, bool isAll, bool hasNormals, bool meshed, bool transformFile
  )
  : fileName_ (file)
  , handler_ (handler)
  , isDir_(isDir)
...
run ()
{
  ...
  if (isDir_)
  {
    ...
    if (_min > 0)
    {
      ...
      cout<<"fileIter->string()="<<fileIter->string()<<endl;
  ...
  else if (isAll_) //load all files from a given directory
  {
    copy(directory_iterator(path(fileName_)),directory_iterator(),back_inserter(files));
    cout<<"files size="<<files.size()<<" _min="<<_min<<endl;
    std::sort(files.begin(),files.end());

    for(auto file : files)
    {
      CloudPtr tmpCloud = CloudPtr(new Cloud);
      pcl::io::loadPCDFile(file.c_str(),*tmpCloud);

      clouds.push_back(tmpCloud);
      cloudNames.push_back(file.string());
      handler_.setInputCloud(tmpCloud);
      cloud_viewer_->addPointCloud(tmpCloud,handler_,file.string());
    }
  }
...
main (int argc, char ** argv)
{
  ...
  if (
    ...
    find_switch (argc, argv, "-dirAll") )
    ||
    (( find_switch (argc, argv, "-pcd") +
    find_switch (argc, argv, "-dir") +
    find_switch (argc, argv, "-transFile") +
    find_switch (argc, argv, "-dirAll") ) > 1) //can only be one of these
  )
  {
    usage (argv);
    return (0);
  ...
  else if (find_switch (argc, argv, "-dirAll"))
  {
    parse_argument (argc, argv, "-dirAll", pcdFile);
  }
